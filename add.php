...
if (!$connection) {
    ...
    if ('addManager' == $action) {
        ...
        if ($b_num && $d_name && $d_description && $d_quantity && $mnf_date && $exp_date) {
            $query = "INSERT INTO drugs(batch_num,drug_name,drug_description,drug_quantity,manufacturing_date,expiry_date,status) VALUES('$b_num','$d_name','$d_description',$d_quantity,'$mnf_date','$exp_date','$status')";
            mysqli_query($connection, $query);
